...
#include "migration/impl/SuccessorMigration.hpp"
...
void
SuccessorMigrator::runMigration(std::shared_ptr<Backend> const& backend, util::config::ObjectView const& config)
{
    ...
    migration::cassandra::impl::SuccessorScanner scanner(
        {.ctxThreadsNum = ctxFullScanThreads, .jobsNum = jobsFullScan, .cursorsPerJob = cursorPerJobsFullScan},
        migration::cassandra::impl::SuccessorAdapter(
            backend,
            [&](std::string const& key, std::uint32_t seq, std::string const& next) {
                backend->writeNewSuccessor(key, seq, next);
            }
        )
    );
    scanner.wait();
